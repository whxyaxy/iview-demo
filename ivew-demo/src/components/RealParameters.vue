<template>
  <div class="home">
    <div class="selectBox">
      <ul><li>机组：</li><li v-for='(item,index) in reportList' :class="{'reportActive':reportSelect == index}" @click="reportIndex(index)">{{item}}</li></ul>
    </div>
    <div class="tableBox">
      <Table  :columns="columns5" :data="data5"></Table>
      <div class="pageBox"><Page :total="100"></Page></div>
    </div>
  </div>
</template>
...
<style scoped>
.ivu-table-header .ivu-table th { background-color: #FCECEC !important;}
...
.pageBox{text-align: right;margin:20px 0 0;}
